...
% 先不进行归一化
% 1. Domain C data
load([datapath,'CWRU_C_Features.mat'],'Features','Features_Normal4T','Labels');
TrainS3 = Features;
LabelS3 = Labels;
C=Features;
      ...
      model_RBF3 = svmtrain(all_labels{3}',mapped_trainS{3}', options);
       model_RBF4 = svmtrain(all_labels{4}',mapped_trainS{4}', options);
           model_RBF = svmtrain(TrainingLabel,TrainingData, options);
            model_RBF = svmtrain(TrainingLabel,TrainingData, options);
        mmd_XY1=my_mmd(C,E,33);
           mmd_XY2=my_mmd(C,F,33);
           mmd_XY3=my_mmd(C,G,33);
           mmd_XY4=my_mmd(C,H,33);  
%            mmd_XY5=my_mmd(TrainingData,mapped_TestData,1);
%             mmd_XY5=my_mmd(mapped_TestData,mapped_trainT',R);
            values=Untitled5(mmd_XY1,mmd_XY2,mmd_XY3,mmd_XY4,100);
