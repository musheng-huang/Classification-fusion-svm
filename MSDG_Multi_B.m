...
% 先不进行归一化
% 1. Domain B data
load([datapath,'CWRU_B_Features.mat'],'Features','Features_Normal4T','Labels');
TrainS2 = Features;
LabelS2 = Labels;
B=Features;
      ...
      model_RBF3 = svmtrain(all_labels{3}',mapped_trainS{3}', options);
       model_RBF4 = svmtrain(all_labels{4}',mapped_trainS{4}', options);
           model_RBF = svmtrain(TrainingLabel,TrainingData, options);
            model_RBF = svmtrain(TrainingLabel,TrainingData, options);
          mmd_XY1=my_mmd(B,E,33);
           mmd_XY2=my_mmd(B,F,33);
           mmd_XY3=my_mmd(B,G,33);
           mmd_XY4=my_mmd(B,H,33);
